...
const SAVE_KEY = "konbuRpgSaveData_v9";
...
function executeAttack(multiplier = 1, isSkill = false) {
    ...
    if (currentEnemy.hp <= 0) {
        ...
        mercsAttack();
        if (currentEnemy.hp > 0) setTimeout(enemyTurn, 600);
...
function onEnemyDefeated() {
    logMessage(`${currentEnemy.name} 討伐！`, "system");
    let expM = 1 + (state.kamuiUpgrades.expBonus * 0.2);
    let goldM = 1 + (state.kamuiUpgrades.goldBonus * 0.2);
    let expG = Math.floor(10 * Math.pow(1.05, state.floor) * expM);
    let goldG = Math.floor(5 * Math.pow(1.05, state.floor) * goldM);
...
function startBattle() {
    ...
    if (!currentEnemy) {
        ...
        updateEnemyHP(); logMessage(`${currentEnemy.name} 出現！`, "system");
    }
...
function generateEnemy(floor) {
    ...
    return { name: boss ? `[BOSS] ${t.name}` : t.name, maxHp: Math.floor(30*t.hpMult*m), hp: Math.floor(30*t.hpMult*m), atk: Math.floor(8*t.atkMult*m), def: Math.floor(3*t.defMult*m), isBoss: boss, image: t.image };
}
...
function generateLoot(floor) {
    ...
    i.name = `[Lv.${fl}] ${p}${names[type]}`;
    if (type === 'weapon') i.atk = stat;
    else if (type === 'armor') i.def = stat;
...
function updateBattleControls() {
    ...
    else { proceedCtrl.classList.add("hidden"); if (!state.isAutoMode) { manualCtrl.classList.remove("hidden"); document.getElementById("btn-attack").disabled = isActing; document.getElementById("btn-open-skills").disabled = isActing; } else { manualCtrl.classList.add("hidden"); } }
}
